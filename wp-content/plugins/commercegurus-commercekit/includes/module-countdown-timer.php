...
@media (max-width: 500px) { #commercekit-timer.product { display: block; width: 100%; float: none; } #commercekit-timer.product.has-cg-inventory { 
border: none;}}
</style>
<script>
...
function commercekit_countdown_timer_by_location( $location ) {
	...
	if ( 'checkout' === $location ) {
		if ( isset( $countdown['checkout']['active'] ) && 1 === (int) $countdown['checkout']['active'] ) {
			$title            = commercekit_get_multilingual_string( $countdown['checkout']['title'] );
			$message          = commercekit_get_multilingual_string( $countdown['checkout']['expiry_message'] );
			$timer['total']   = ( (int) $countdown['checkout']['minutes'] ) * 60;
			$timer['total']  += (int) $countdown['checkout']['seconds'];
			$timer['days']    = 0;
			$timer['hours']   = 0;
			$timer['minutes'] = (int) $countdown['checkout']['minutes'];
			$timer['seconds'] = (int) $countdown['checkout']['seconds'];
		} else {
			return;
		}
	}
