...
function shoptimizer_scripts() {
	...
	wp_enqueue_script( 'shoptimizer-main', get_template_directory_uri() . '/assets/js/main.min.js', array(), '20161208', true );
	...
	if ( shoptimizer_is_woocommerce_activated() ) {
		if ( is_product() || is_cart() ) {
			wp_enqueue_script( 'shoptimizer-quantity', get_template_directory_uri() . '/assets/js/quantity.min.js', array(), '1.1', true );
		}
	}
...
function shoptimizer_register_required_plugins() {
	$plugins = array(
		...
		array(
			...
			'source'   => 'https://files.commercegurus.com/commercekit/1.1.8/commercegurus-commercekit.zip',
			'required' => true,
			'version'  => '1.1.8',
		),
		array(
