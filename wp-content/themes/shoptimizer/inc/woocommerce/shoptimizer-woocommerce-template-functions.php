...
if ( ! function_exists( 'shoptimizer_shop_remove_pagination' ) ) {
	 ...
	 * @return  void
	 */
	function shoptimizer_shop_remove_pagination() {

		$shoptimizer_layout_woocommerce_display_pagination = '';
		$shoptimizer_layout_woocommerce_display_pagination = shoptimizer_get_option( 'shoptimizer_layout_woocommerce_display_pagination' );

		if ( false === $shoptimizer_layout_woocommerce_display_pagination ) {
			remove_action( 'woocommerce_before_shop_loop', 'shoptimizer_woocommerce_pagination', 30 );
		}
	}
}

add_action( 'after_setup_theme', 'shoptimizer_shop_remove_pagination', 85 );
...
if ( ! function_exists( 'shoptimizer_pdp_ajax_atc_notices' ) ) {
	/**
	 * PDP/Single product ajax add to cart error notices.
	 */
	function shoptimizer_pdp_ajax_atc_notices() {
		wc_print_notices();
	}
}
...
if ( true === $shoptimizer_layout_woocommerce_single_product_ajax ) {
	add_action( 'woocommerce_before_single_product', 'shoptimizer_pdp_ajax_atc_notices', 10 );
...
function shoptimizer_category_image() {
	if ( is_product_category() ) {
		...
		if ( $image ) {
			echo '<img src="' . $image . '" alt="' . $alt_txt . '" />';
		}
	}
}
